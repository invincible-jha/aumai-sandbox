...
class NetworkEgressRule(BaseModel):
    ...
    domain: str = Field(..., description="FQDN or wildcard prefix, e.g. '*.example.com'")
    ports: list[int] = Field(default_factory=list, description="Allowed TCP/UDP ports; empty = all")
...
class FilesystemConfig(BaseModel):
    ...
    writable_paths: list[str] = Field(
        ...
        description="Absolute paths the sandbox may write to; only meaningful when mode=read_write",
    )
...
class ResourceLimits(BaseModel):
    ...
    max_memory_mb: int = Field(default=512, ge=1, description="Maximum resident memory in MiB")
...
class SandboxResult(BaseModel):
    ...
    duration_ms: float = Field(..., ge=0.0, description="Wall-clock execution time in ms")
    resource_usage: dict[str, Any] = Field(
        default_factory=dict,
        description="Observed resource metrics: cpu_seconds, peak_memory_mb, tokens_used, cost_usd",
    )
